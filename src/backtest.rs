...
// estimate_historical_volatility, num_of_weeks, compute_rate_of_return
// remain in backtest because they are backtest-domain utilities
// (candle-based vol estimation, Tiger-like return formula). The pure
// BS greeks (cumulative_normal, black_scholes_put, put_delta, etc.)
// live in `greeks.rs` and are used via `crate::greeks::*`.
    ...
    for sim_date in &mondays {
        ...
        for symbol in symbols {
            ...
            while strike <= extended_ceiling {
                ...
                let premium = greeks::black_scholes_put(
                    price,
                    strike,
                    t,
