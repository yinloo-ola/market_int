# Backtest Subcommand — Implementation Plan

Design doc: `docs/plans/2026-05-22-backtest-design.md`

## Task 1: Extract pure indicator functions from existing modules
...
- `src/maxdrop.rs`
- `src/sharpe.rs`
- `src/price_percentile.rs`

Steps:

1. Run `cargo test` to confirm all existing tests pass before changes.

2. In `src/maxdrop.rs`, make `calculate_max_drop` public and add a new public function `compute_max_drop_stats`:
...
Then refactor `calculate_and_save` to call it:

```rust
// Replace the block starting at "let max_drops: Vec<f64> = candles
//     .chunks(period)" through the end of the if/else with:
...
match compute_max_drop_stats(&candles, period) {
    Some((percentile_drop, ema_drop)) => {
        ...
        store::max_drop::save_max_drop_period(
            conn, &symbol, period, percentile_drop, ema_drop, timestamp,
        )?;
        log::info!(
...
Remove the now-redundant `let timestamp = candles.last().unwrap().timestamp;` line that was before the old block (it's now inside the match arm).

3. In `src/sharpe.rs`, make existing functions public and add a convenience wrapper:
...
Make `calculate_returns` and `calculate_sharpe` public (change `fn` to `pub fn`).

4. In `src/price_percentile.rs`, make the calculation function public:
...
Rename `calculate_price_percentile` to `compute_price_percentile` and make it `pub`. Update the call site in `calculate_and_save`:
...
6. Run `cargo check` — no compilation errors.

---

## Task 2: Add `get_candles_up_to` store function
...
1. Add a new query function to `src/store/candle.rs` that returns candles up to (and including) a given timestamp:
...
pub fn get_candles_up_to(
    ...
    let mut rows = stmt.query(params![symbol, as_of, count])?;
...
2. Run `cargo check` — must compile.

---

## Task 3: Add Black-Scholes put pricing with tests

<!-- tdd: new-feature -->
<!-- checkpoint: test -->

Files:
- `src/backtest.rs` (new file)

Steps:

1. Create `src/backtest.rs` with Black-Scholes functions and tests:
...
use crate::{
    atr, constants, maxdrop, model, price_percentile, sharpe, trend,
    store::candle,
    symbols,
    option::calculate_adjusted_strike_range,
};
...
/// Uses rolling window of daily returns, annualized by sqrt(252).
...
mod tests {
    ...
    fn test_bs_put_at_the_money() {
        ...
        assert!(price > 0.0, "put price should be positive, got {}", price);
        assert!(price < 15.0, "put price should be < 15 for 20% vol ATM, got {}", price);
    }
    ...
    fn test_estimate_volatility_trending() {
        // 30 days of 1% daily moves
        let closes: Vec<f64> = (0..31).map(|i| 100.0 * (1.01_f64).powi(i)).collect();
        let vol = estimate_historical_volatility(&closes, 20);
        // Daily log return ≈ ln(1.01) ≈ 0.00995, annualized ≈ 0.00995 * sqrt(252) ≈ 0.158
        assert!(
            (vol - 0.16).abs() < 0.05,
            "vol should be ~16% for 1% daily moves, got {:.4}",
            vol
        );
    }
    ...
    fn test_estimate_volatility_insufficient_data() {
        ...
        let vol = estimate_historical_volatility(&closes, 20);
        assert_eq!(vol, 0.30, "should return default 30% for insufficient data");
    }
...
2. Add `pub mod backtest;` to `src/main.rs` in the module declarations block (after `mod constants;`).

3. Run `cargo test -- backtest::tests` — all 11 tests must pass.

4. Run `cargo test` — all existing tests still pass.

⏸ **CHECKPOINT: test** — review Black-Scholes implementation and test coverage before building the simulation engine.

---

## Task 4: Add BacktestConfig and presets
...
1. Add the `BacktestConfig` struct and preset factory methods to `src/backtest.rs` (after the Black-Scholes section, before `#[cfg(test)]`):
...
impl BacktestConfig {
    ...
    pub fn no_trend_factor() -> Self {
        Self { use_trend_factor: false, name: "no-trend-factor".to_string(), ..Self::control() }
    }

    /// No trend_long pre-filter.
    pub fn no_trend_long() -> Self {
        Self { use_trend_long_filter: false, name: "no-trend-long".to_string(), ..Self::control() }
    }
    ...
    pub fn no_regime() -> Self {
        Self { use_regime: false, name: "no-regime".to_string(), ..Self::control() }
    }
    ...
    pub fn compute_trend_factor(&self, trend_ratio_short: f64) -> f64 {
        ...
        if trend_ratio_short <= peak {
            ...
            let peak_reduction = ((peak - 1.0) * self.trend_tighten_multiplier).min(cap);
            let excess = trend_ratio_short - peak;
            let reduction = (peak_reduction - excess * self.trend_tighten_ease_back).max(0.0);
            1.0 - reduction
        }
    }
    ...
    pub fn score_candidate(
        ...
        // Pre-filters
        if rate_of_return < self.min_rate_of_return || rate_of_return > self.max_rate_of_return {
            return None;
        }
        if sharpe <= 0.0 {
            return None;
...
```rust
    ...
    fn test_score_rejects_below_min_return() {
        ...
        assert!(result.is_none(), "should reject rate_of_return below 0.25");
    }
    ...
    fn test_all_presets_have_valid_names() {
        for config in BacktestConfig::all_presets() {
            ...
            assert!(!config.name.contains(' '), "preset name should not have spaces: {}", config.name);
        }
    }
...
3. Run `cargo test -- backtest::tests` — all tests pass.

4. Run `cargo test` — all tests pass.

---

## Task 5: Add simulation engine — result types and core loop
...
1. Add result types to `src/backtest.rs` (before `#[cfg(test)]`):
...
pub struct BacktestMetrics {
    ...
    pub avg_score: f64,
    pub avg_loss_when_assigned: f64, // average % below strike
...
2. Add the simulation engine function. This is the core of the backtest — it computes indicators from candle slices (no DB writes), prices strikes via Black-Scholes, scores them, and checks assignment:

```rust
// ── Simulation Engine ──────────────────────────────────────────

/// Load all candles for each symbol into memory. Returns a map of symbol → candles (chronological).
fn load_all_candles(
    conn: &Connection,
    symbols: &[String],
...
/// Convert a NaiveDate to unix timestamp (start of day UTC).
fn date_to_timestamp(date: NaiveDate) -> u32 {
    date.and_hms_opt(23, 59, 59).unwrap().and_utc().timestamp() as u32
}
...
/// Returns None if no candle found within 5 trading days.
        ...
        if c.timestamp >= target_ts && c.timestamp <= max_ts {
            let candle_date =
                chrono::DateTime::from_timestamp(c.timestamp as i64, 0)
                    .unwrap_or_default()
                    .date_naive();
            return Some((c.close, candle_date));
        }
    }
    None
}
...
pub fn run_backtest(
    ...
    for sim_date in &mondays {
        ...
        let spy_up_to: Vec<model::Candle> = spy_candles
            .iter()
            .filter(|c| c.timestamp <= sim_ts)
            .cloned()
            .collect();
        ...
        // (symbol_idx, score, strike, rate_of_return, trend_short, trend_long, price, sector)
        ...
        for (symbol_idx, symbol) in symbols.iter().enumerate() {
            ...
            let candles: Vec<model::Candle> = all_candles
                .iter()
                .filter(|c| c.timestamp <= sim_ts)
                .cloned()
                .collect();
            ...
            let (percentile_drop, ema_drop) = match maxdrop::compute_max_drop_stats(&candles, period) {
                Some(stats) => stats,
                None => continue,
            };
            ...
            // Price range for strike percentile
            let pp_candles: Vec<model::Candle> = candles
                .iter()
                .rev()
                .take(constants::PRICE_PERCENTILE_DAYS as usize)
                .cloned()
                .collect::<Vec<_>>()
                .into_iter()
                .rev()
                .collect();
            let range_min = pp_candles.iter().map(|c| c.close).fold(f64::INFINITY, f64::min);
            let range_max = pp_candles.iter().map(|c| c.close).fold(f64::NEG_INFINITY, f64::max);

            // Generate strikes at $0.50 intervals
            let mut strike = (min_strike / 0.5).ceil() * 0.5;
            while strike <= max_strike {
                let t = dte as f64 / 252.0;
                let premium = black_scholes_put(price, strike, t, config.risk_free_rate, config.dividend_yield, vol);
                if premium <= 0.0 {
                    strike += 0.5;
                    continue;
                }
                let rate_of_return = compute_rate_of_return(premium, strike, dte);
                let strike_pct = model::calculate_strike_percentile(strike, range_min, range_max);
                let sector = crate::sectors::sector_of(sectors, symbol);

                match config.score_candidate(
                    sharpe_ratio, strike_pct, rate_of_return, trend_short, trend_long, &regime,
                ) {
                    Some(score) => {
                        candidates.push((symbol_idx, score, strike, rate_of_return, trend_short, trend_long, price, sector));
                    }
                    None => {}
                }
                strike += 0.5;
        ...
        for (symbol_idx, score, strike, ror, ts, tl, price, sector) in &top_picks {
            ...
            let (close_expiry, close_day_after, assigned) = match candles_map.get(symbol) {
                Some(candles) => {
                    let ec = find_close_on_date(candles, expiry_date);
                    let dac = find_close_on_date(candles, day_after);
                    let assigned = ec.map(|(c, _)| c < *strike).unwrap_or(false)
                        || dac.map(|(c, _)| c < *strike).unwrap_or(false);
                    (ec.map(|(c, _)| c), dac.map(|(c, _)| c), assigned)
                }
    ...
    let avg_loss = picks
        ...
        .collect::<Vec<_>>();
    let avg_loss_when_assigned = if avg_loss.is_empty() {
        0.0
    } else {
        avg_loss.iter().sum::<f64>() / avg_loss.len() as f64
    };
...
3. Run `cargo check` — must compile. Fix any type errors. Note: `model::Candle` needs `#[derive(Debug, Clone)]` — add `Clone` to the `Candle` struct in `src/model.rs` if it's not already there.

4. Run `cargo test` — all tests pass.

⏸ **CHECKPOINT: done** — review the simulation engine logic before proceeding to output formatting and CLI wiring.

---

## Task 6: Add output formatting and CSV export
...
1. Add terminal summary formatting and CSV writing to `src/backtest.rs` (before `#[cfg(test)]`):
...
pub fn format_metrics(metrics: &BacktestMetrics) -> String {
    ...
    // Regime breakdown
    let bull_picks: Vec<&BacktestPick> = metrics.picks.iter().filter(|p| p.regime_flag.is_empty()).collect();
    let corr_picks: Vec<&BacktestPick> = metrics.picks.iter().filter(|p| p.regime_flag.contains("Correction")).collect();
    let bear_picks: Vec<&BacktestPick> = metrics.picks.iter().filter(|p| p.regime_flag.contains("Bear")).collect();
    ...
    if !bull_picks.is_empty() || !corr_picks.is_empty() || !bear_picks.is_empty() {
        ...
        if !bull_picks.is_empty() {
            ...
            let avg_ror = bull_picks.iter().map(|p| p.rate_of_return).sum::<f64>() / bull_picks.len() as f64;
            out.push_str(&format!(
        ...
        if !corr_picks.is_empty() {
            ...
            let avg_ror = corr_picks.iter().map(|p| p.rate_of_return).sum::<f64>() / corr_picks.len() as f64;
            out.push_str(&format!(
        ...
        if !bear_picks.is_empty() {
            ...
            let avg_ror = bear_picks.iter().map(|p| p.rate_of_return).sum::<f64>() / bear_picks.len() as f64;
            out.push_str(&format!(
...
pub fn write_csv(path: &str, all_metrics: &[BacktestMetrics]) -> model::Result<()> {
    use std::io::Write;
    let mut file = std::fs::File::create(path).map_err(|e| model::QuotesError::CsvError(e))?;
    // Header
    writeln!(
        file,
    ...
    ).map_err(|e| model::QuotesError::CsvError(e))?;
    for metrics in all_metrics {
        for pick in &metrics.picks {
            writeln!(
                ...
                pick.regime_flag,
                pick.assigned,
                pick.close_at_expiry.map(|v| format!("{:.2}", v)).unwrap_or_default(),
                pick.close_day_after.map(|v| format!("{:.2}", v)).unwrap_or_default(),
            ).map_err(|e| model::QuotesError::CsvError(e))?;
        }
    }
    Ok(())
...
## Task 7: Wire backtest subcommand into CLI
...
1. Add the `Backtest` subcommand variant to the `Commands` enum in `src/main.rs`:
...
2. Add the match arm for `Commands::Backtest`:
...
```rust
        Commands::Backtest {
            ...
            let configs: Vec<crate::backtest::BacktestConfig> = if config == "all" {
                crate::backtest::BacktestConfig::all_presets()
                    .into_iter()
                    .map(|mut c| {
                ...
                match crate::backtest::BacktestConfig::by_name(&config) {
                    Some(mut c) => {
                        if let Some(p) = period {
                            c.period = p;
                        }
            ...
            for cfg in &configs {
                ...
                let metrics = crate::backtest::run_backtest(
                    cfg, &conn, &symbols, &sectors, from_date, to_date,
                );
                println!("{}", crate::backtest::format_metrics(&metrics));
                all_metrics.push(metrics);
            }

            match crate::backtest::write_csv(&output, &all_metrics) {
...
Note: This requires `use chrono::NaiveDate;` at the top of `main.rs` (it's already imported via `use chrono::{Datelike, Local};` — just add `NaiveDate` to that import).

3. Run `cargo check` — must compile.
...
1. Run a quick smoke test against real data:

```bash
RUST_LOG=info cargo run -- backtest data/symbols.csv --from 2024-01-01 --to 2024-03-31 --config control --output /tmp/backtest_smoke.csv
```

Expected: terminal output showing the `control` config metrics, CSV file at `/tmp/backtest_smoke.csv` with rows. The run should complete in seconds since it's all local computation.

2. Verify the CSV has data:

```bash
head -5 /tmp/backtest_smoke.csv
wc -l /tmp/backtest_smoke.csv
```

Expected: header row + at least a few data rows (depending on how many Mondays had qualifying picks).

3. Run `cargo test` — all tests still pass.

4. Update `docs/lessons.md` if any lessons were learned during implementation.
