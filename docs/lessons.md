...
## Validation Rules (backtests / signal research)

- **A within-sample p-value never establishes generalization.** A permutation / significance test that holds the window's data fixed answers only "is this noise *given these returns*?" — it cannot detect a regime coincidence (an effect real in this period but not beyond it). Always pair an in-window significance test with a **walk-forward / held-out-regime test** that evaluates frozen parameters across windows the calibration never saw. The direction-signal ranking had within-window p=0.0001 yet collapsed to t=0.32 over the full history — the permutation test was correct *about its narrow question* and misleading about the one that mattered.
- **Validate on data the parameters never touched, in a regime unlike the one they were tuned in.** Reusing the same names and an adjacent time window mostly retests the same conditions. A genuine factor survives unseen regimes with frozen weights; one that needs per-regime refit has no deployable edge.
- **A positive headline from a single test split is a hypothesis to confirm, not an edge to deploy.** Before investing in a promising single-window result, ask which *threats* remain (sampling noise vs regime-generalization vs selection bias) and run the cheapest test that addresses each before the expensive one — ordering matters because a cheap negative makes the expensive test moot.
