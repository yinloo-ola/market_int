# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Added
- **Extended-hours quotes for lots (webapp + core, 2026-09-26)** — a lot's `spot` is now the **latest known price**: outside regular hours a refresh adds up to three batched 1-minute kline calls via the kline gateway's `trade_session` parameter (PreMarket / AfterHours / OverNight — live-verified under the account's existing permissions after the real-time `brief` snapshot proved paywalled: the gateway method is `brief` (not `quote`), needs `usQuoteBasic`, and the account holds only `aStockQuoteLv1` + `usOptionQuote`; the free delayed brief carries no extended data), and the chronologically newest close becomes `spot` with `session` naming its source — value and P&L follow it, the way a broker app prices shares over the weekend (Saturday shows the 04:00 ET overnight close). A market-session classifier (`et_market_session`, ET wall clock via chrono-tz — DST-safe, Blue-Ocean overnight shape with no Friday/Saturday-night session, live-probed) gates the calls; regular-session refreshes clear the extended data and price from the live regular pass exactly as before. Ledger additive per ADR-0002 (`pre`/`post`/`overnight`/`session`, serde defaults); the row shows one price tagged with its source session (`pre`/`post`/`overnight`). `make test-quote SYMBOLS=AAPL` probes the live gateway. Smoke 127/127; cargo 25/166/104.
- **Manual lot close + called-away proceeds (webapp + core, 2026-09-26)** — `close…` on a lot row opens an inline form (shares prefilled to all, sale price, live proceeds preview naming the pool); `POST /api/holdings/close {lot_id, shares, price}` reduces or removes the lot and credits `price × shares` to the lot's pool in one rewrite (404/400 with reasons and no write on error; dangling `pool_id` falls back to the default pool). **Called-away now also credits** `strike × 100 × contracts` to the call's pool in the same rewrite (supersedes the no-credit rule; the wheel E2E re-pinned). Pool-consistency pass (pools = broker accounts; money and shares physically stay in one account): **lot creation debits its cost** (shares × basis) from a chosen pool (dropdown at 2+ pools); the assignment debit follows the put's pool with no pool choice; close/sell-call pool overrides were removed (proceeds and called-away scoping always follow the position's own pool); the **lot pill became a live picker** (PATCH extended to lots — a field rewrite, no stored cash moves) and the manual CALLS-tab form takes a pool. Mobile: pool controls align right on every row head. Smoke 127/127; cargo 25/166/104.
- **Cash pools (webapp + core, 2026-09-23)** — the Holdings ledger tracks named cash pools (one per broker account): a `cash_pools` array with per-pool `cash`/`reserved`/`free` plus `pool_id` on puts, calls and lots (schema stays **v1** per ADR-0002 — `#[serde(default)]` siblings, legacy scalar `cash` reads as one implicit "Main" pool, materialized on first write, and stays mirrored to the pool sum for rollback). Puts reserve strike×100×contracts against their own pool (`pool_id: None` folds into the first); assignment records the lot in the put's pool and moves strike×100 out of that pool's cash in the same rewrite; called-away FIFO is pool-scoped. Pool CRUD: `POST /api/holdings` `kind:"pool"`, pool-aware `PATCH /api/holdings/cash` (`{pool_id?, cash?, name?}`, missing id = first pool), guarded pool DELETE (409 while entries reference it or it's the default pool with unassigned entries; a funded pool deletes freely — its recorded cash leaves the aggregate with it, and the last pool's delete falls back to the scalar shape keeping its balance), 20-pool cap, 80-char names. Rail UI: an always-visible pool list — each row shows its cash figure with edit-cash/rename/delete inline, picking a row points the strip's free/cash figures at that pool, and the add row is offered even before the first pool exists; the put form carries a pool dropdown and every put row a small tag naming the pool its reservation spends (`pool_name`, server-resolved — `None` folds to the default pool client-visibly never). Smoke lane extended to 104 assertions (pool list walk + zero-pool add + put-form pool submit + row pool tag); core gains `reserved_cash_by_pool`.
- **Dark mode (webapp frontend, 2026-09-23)** — the app follows the system light/dark setting (`prefers-color-scheme`) and a header sun/moon toggle (beside ▶ Run pipeline) forces the opposite theme for the session via `html[data-theme]`; deliberately non-persistent — a reload returns to system-follow. Enabler: all 67 hardcoded hex values in `style.css` were tokenized into CSS custom properties first, so both themes are pure variable re-declarations (a dark media-query block plus explicit `data-theme="dark"`/`"light"` attribute blocks; most-specific-wins) and light rendering is unchanged by construction. Smoke lane extended to 87 assertions (stylesheet-level palette checks + toggle behavior, colors themselves being beyond happy-dom); backend untouched.
- **Wheel holdings (webapp + core, 2026-09-17)** — the holdings ledger now tracks the full wheel beside sold puts: **covered calls**, **share lots from assignments**, and a **manual cash balance** with derived `cash_reserved` (Σ strike×100×contracts of open puts) and `cash_free` (honest negatives). One refresh marks all three kinds: per-side chain queries (the Tiger call side's first production exercise — **verified live 2026-09-18**: Tiger returns ITM call rows for the degenerate single-strike query despite the request's `in_the_money: false` — SPY spot 762.60, 750C returned mid 13.32, OTM 775C mid 0.515; `make test-tiger --side call --strike N` reproduces) plus one underlying quote per unique symbol pricing lots through the `MarkBatch{marks, spots}` seam. Wheel transitions are single-rewrite server mutations: assignment (`kind:"lot"` + `assigned_from`) records the lot and removes the put atomically; `POST /api/holdings/called-away` removes the call and FIFO-reduces the earliest-acquired covering lot (ties by id), a 200 `reduced:false` outcome when no lot covers. Ledger schema stays **v1** — additive `#[serde(default)]` fields only; rollback past this feature erases calls/lots/cash on next write (see `docs/adr/0002-holdings-ledger-additive-schema-version-stays-1.md`). Frontend: variant-C panel — sticky rail (cash strip PATCHing `/api/holdings/cash` and re-rendering from the response, wheel stats, lot rows with a `sell call…` prefill at floor(shares/100)), one merged urgency list (kind chips, pace bars, `buy back?` / `ITM — called away?` chips, full stats line), every dialog anchored inline under its position, assignment prefill (shares = contracts×100, basis = strike − premium). Smoke lane extended to 63 assertions; scoring-parity lane untouched.

- **Holdings management (webapp, 2026-09-11)** — a per-user "Holdings" tab: enter the puts you sold (symbol, strike, expiry, premium, contracts, sell date), mark them to market on demand with Tiger bid/ask mids (degenerate strike-range chain query), and read the close decision off each card — sold at / now (mid) / close captures $ — plus the underlying spot price with its %-vs-strike (danger-colored when the short put is ITM). Pace rule with a 1-day floor: `target% = max(elapsed working days, 1) / total working days × 100`, "buy back?" flag when profit% ≥ target%. Open positions only: recording bought-back/expired/assigned shows realized P&L and removes the position. Ledger: one schema-versioned JSON document per verified Firebase UID on the GCS mount (see `docs/adr/0001-holdings-per-user-json-ledger-on-gcs.md`); no new dependencies; no manifest change (routes + panel ship in the same image).

- **`ScoreParams` struct** in `model.rs` — encapsulates all tunable scoring parameters (`min_rate_of_return`, `ideal_return`, weights, trend-score thresholds, earnings safety multiplier) so the backtest's production-mirror delegate path respects config overrides. Threaded through `calculate_put_score` and `calculate_put_chain_score`. Default-initialized from constants.
- **11 new backtest presets**: `pm-ideal-085/090/095/100`, `pm-min-ror-30/35/40`, `pm-safety-50`, `pm-return-50`, `pm-vol-high-ideal-090`, `pm-vol-high-safety-50` — production-mirror ablation sweeps that now produce distinct results (fixed by ScoreParams).

### Changed

- **Holdings tab → side-rail sub-tabs (webapp frontend, 2026-09-19)** — the variant-C panel (sticky rail + one merged urgency list) is replaced by the prototype-approved **variant B "Side rail"** (user verdict over "Segment bar" and "Page hero"): `LOTS | PUTS | CALLS` tile tabs in a sticky rail — a horizontal tile strip below 900px — beside a pane column rendering only the active kind, urgency-sorted per kind with the same key the merged list ranked. Cash strip, Refresh marks and the flash notice stay reachable from every tab; counts + context lines ("N sh held" / "N pace-met" / "N ITM") live on the tiles, replacing the wheel-stats block; add buttons and forms are pane-local; dialogs still anchor inline under their entry. Backend contract unchanged (same five `/api/holdings` endpoints, ledger schema untouched). The panel fills the app shell (the standalone prototype's page geometry — max-width/centering — fought the shell and was dropped). Smoke lane rewritten as the feature E2E over a mutable mocked ledger: 79 assertions. Prototype preserved on branch `prototype/holdings-subtabs`.
- **vite dev allows `*.trycloudflare.com` hosts** — cloudflared quick tunnels randomize the hostname per run; without the allow-list, vite 6 403s every tunnel request (the tunnel/phone-preview flow: `FIREBASE_PROJECT_ID=` disarmed server + `VITE_PREVIEW_USER` vite dev; the disarmed server keys holdings as `local`).

- **`MIN_RATE_OF_RETURN` raised from 0.25 to 0.30** — backtest-validated as a strict improvement: assignment rate 1.94% vs 2.18%, avg ROR 64.8% vs 61.7% (3-year sweep, 157 sim weeks, 413 picks per preset).
- **Vol-tier annotation (D2, 2026-07)** — each published put pick is annotated with a volatility tier in the Telegram caption (`🟢` vol ≥ 0.38, `🟡` 0.28–0.38, `🔴` < 0.28) and a numeric value, plus a new `realized_vol` column in the published option-chain CSV. Surfaces capital-efficiency context (high-vol names deliver materially higher `rate_of_return` at matched assignment rate, per the 1.21M-observation calibration run) **without filtering** — the bot's output is a research candidate pool, not a trade list.
- **`backtest --calibrate-safety`** — read-only mode that walks every strike across the production-mirror band (plus an extension toward spot) and prints the empirical `safety → breach-rate` curve, bucketed by regime / earnings-window / trend-strength, with Wilson 95% CIs. Emits a `safety_calibration.csv` for offline analysis. Produced the dataset that validated (and falsified) the safety-recalibration and trend-term hypotheses.
- **`trend_short` parameter** on `calculate_put_score` / `calculate_put_chain_score`, with a trend term wired into the composite score. Disabled by default (`PUT_SCORE_WEIGHT_TREND = 0.0`): the 2026-07 sweep found every non-zero weight (0.03–0.15) lifted assignment rate above the 2.4% baseline. The lever is parked, documented with the evidence, and pinned by `test_put_score_trend_term_disabled_by_default`.
- **6 new backtest presets**: `vol-mid-plus` (vol ≥ 0.28), `vol-median-plus` (vol ≥ 0.32), `vol-high-only` (vol ≥ 0.50, research-only hard filter), `picks-4` / `picks-5` / `picks-6` (raised pick-count caps). Total presets: 38 → 46.
- **Named weight constants** in `src/constants.rs`: `PUT_SCORE_WEIGHT_SHARPE` (0.20), `PUT_SCORE_WEIGHT_SAFETY` (0.40), `PUT_SCORE_WEIGHT_RETURN` (0.40), `PUT_SCORE_WEIGHT_TREND` (0.0); plus `TOP_PICKS_COUNT` (3), `TREND_SCORE_FLOOR` (1.02), `TREND_SCORE_BAND` (0.06), `MIN_REALIZED_VOL` (0.50, backtest-only).
- **Earnings-aware put scoring** — when a symbol reports earnings inside `[today, expiry]`, upper-half strikes (`strike > band midpoint`) are excluded and surviving lower-half strikes score with halved safety (`EARNINGS_SAFETY_MULTIPLIER = 0.5`); the max_drop band is built from non-event volatility. New pure helpers `calculate_put_chain_score` / `earnings_in_window`; `calculate_put_score` is unchanged.
- **Earnings calendar persisted** (`store::earnings`) — the live run writes a snapshot so the offline `publish-option-chain` re-publish path applies the same earnings rule as the live run.
- `fetch-earnings <from> <to>` subcommand — materializes the Tiger earnings calendar for a date range to a CSV that feeds `backtest --earnings`.
- `backtest --earnings <csv>` flag; `production_mirror` now delegates to `calculate_put_chain_score` (earnings-aware) so the backtest mirrors production exactly when earnings data is supplied. Absent the flag, behavior is unchanged.
- Added support for multiple options-scoring paradigms in the backtester (`Symmetric`, `AsymmetricStatic`, and `AsymmetricDynamic`) to test different yield targets
- Added Net P&L per share, total premium collected, and total assignment loss tracking to backtest metrics
- Deployed highly profitable `premium-static-080` scoring model in production, shifting `IDEAL_RETURN` to 80% with an asymmetric soft-cap and a protective `0.40` maximum strike percentile safety ceiling

### Fixed

- Top-pick sort no longer panics on a `NaN` score (e.g. corrupt strike-band data): `partial_cmp().unwrap()` → `unwrap_or(Equal)`.
- Earnings-window `today` is now derived in New York time to match the earnings-calendar fetch (was `Local` = UTC on Cloud Run, drifting ±1 day at the boundary).
- `load_earnings` accepts both headered and headerless earnings CSVs, and its docstring no longer misstates the missing-file behavior (it returns `Err`; the caller falls back to earnings-blind).
- Fixed a backtest assignment bug where weekend option expiry dates (Saturday/Sunday) skipped the Friday close and checked next Monday's close, falsely inflating assignments due to weekend gaps.
- Adjusted all corresponding scoring unit tests in `src/model.rs` to align with the new 80% asymmetric soft-cap math, returning the entire test suite to 100% green.

### Changed

- `publish_to_telegram`'s `_earnings_map` renamed to `earnings_map` — it now drives scoring (via `calculate_put_chain_score`), not just the display warning.
- Lowered `MIN_RATE_OF_RETURN` from 0.30 to 0.25 to increase search-space flexibility, and lowered `MAX_STRIKE_PERCENTILE` from 0.60 to 0.40 to guarantee deeply out-of-the-money safety margins.
- **Put-option safety is now derived from the max_drop band** (`calculate_max_drop_safety` over `[strike_from, strike_to]`) instead of the 20-day `strike_percentile`. Deep strikes (rarely-breached historical drops) now score highest; the score and the strike-range filter agree on what "safe" means.
- `PERCENTILE` raised 0.9 → 0.97 — wider strike bands (median 5-day band ~2.4% → ~5.3%), fixing narrow-band cases where calm stocks had no candidate strikes.
- Backtest: added `SafetySource` (`StrikePercentile` / `MaxDropBand`) and a `production-mirror` preset that faithfully reproduces live scoring (pinned by a regression test).

### Removed

- The `rate_of_return > 0.80` hard cap and the `strike_percentile > 0.40` pre-filter from put scoring. `rate_of_return` is now a pure soft-capped reward (no upper exclusion); danger is expressed entirely via the max_drop band. `strike_percentile` remains in the CSV as a diagnostic.

## [0.7.0] - 2026-05-16

### Added

- Market regime-aware trend filtering — SPY's position relative to its EMA50 dynamically adjusts the trend filter threshold (0.98→0.92) and scoring weights (safety↑ trend↓) based on market conditions (#98ddf1f, #9c3da59)
- Telegram caption shows ⚠️ Correction or 🐻 Bear market flag when SPY is below its EMA50 (#f0e756a)
- 15 new tests covering regime calculation, dynamic scoring, and integration scenarios (#98ddf1f, #b666d23)

### Fixed

- Top picks now always select different underlying stocks — no more duplicates when one stock dominates the score rankings (#5455581)

## [0.6.0] - 2026-05-10

### Added

- Earnings calendar integration via Tiger OpenAPI — flags upcoming earnings in pipeline output (#7463bbb)
- `momentum_flag` helper with thresholds: NORMAL / HIGH / EXTENDED (#3c21bbb)
- Earnings date and momentum flag columns in CSV output (#6a0b7b1)
- TopPick selection column highlighting best put opportunities (#6a0b7b1)
- Telegram caption with top 3 puts, momentum warnings, and earnings alerts (#88e855b)
- Price percentile calculation collected from DB and shown in caption (#7e6c323, #0ab3b7e)

### Changed

- Widened scoring pre-filter thresholds to `return >= 0.25`, `percentile <= 0.60` (#9f7ab4a)
- Renamed CSV column to `price_percentile` for clarity (#26326e7)

## [0.5.6] - 2026-05-09

### Changed

- Removed `last`, `updated`, and `dte` columns from CSV output for cleaner reports (#42ce5c8)
- Cleaned up internal logic and simplified option pipeline implementation (#332ffd9)
- Increased strike percentage minimum to `0.5` for safer put selection (#b81cff8)
- Adjusted rate of return bounds in put score calculation (#1cf3794)

## [0.5.5] - 2026-05-05

### Added

- Strike percentile calculation with storage in SQLite (#c74080e)
- Put scoring function with configurable weights and unit tests (#c74080e)
- Strike percentile and put score wired into CSV output (#a68f535)

## [0.5.4] - 2026-02-07

### Added

- Price percentile calculation and persistent storage (#2a8136d)

## [0.5.3] - 2026-01-01

### Changed

- Added caching for option expiration data in Tiger API requester (#48cdd7d)

## [0.5.2] - 2025-12-20

### Changed

- Added option chain filtering configuration and logic (#2cf4223)
- Enhanced trading days calculation and adjusted strike range logic (#4cec18a)

## [0.5.1] - 2025-12-12

### Changed

- Updated safety range calculation in option chain retrieval (#5469046)
- Moved period calculation outside loop and added timezone configuration (#a073361)

## [0.5.0] - 2025-11-29

### Added

- Calculate max drop for both 5-day and 20-day periods (#99a7762)

### Changed

- Refactored database table creation and code structure (#83bd362)

## [0.4.0] - 2025-10-03

### Changed

- Replaced ATR with max drop to favour stocks with less frequent drops (#6dce368)
- Improved EMA calculation (#9246937)

### Fixed

- Fixed `max_drop` unique index constraint (#199d460)

## [0.3.0] - 2025-09-17

### Added

- Filter out low-quality option chains (wide spreads, missing data) (#caae129)
- Underlying price support in option chain queries (#90aa1fa)

### Changed

- Migrated from MarketData to Tiger OpenAPI for options chain data (#1a3cdf4, #b9f3923, #8d0a255)

### Fixed

- Formatted numeric values to three decimal places (#2dc0333)

## [0.2.0] - 2025-09-09

### Added

- Tiger OpenAPI integration with RSA key authentication (#bae6fce, #8d0a255)

### Fixed

- Fixed Tiger RSA key encoding (#55c438b)

## [0.1.0] - 2025-07-05

### Added

- Sharpe ratio calculation and persistent storage (#36243f2)

### Fixed

- Improved Sharpe ratio calculation and reporting (#7618f47)

## [0.0.3] - 2024-10-29

### Changed

- Switched from absolute true range to ratio-based calculation (#eca9ee8, #23b7844)

## [0.0.2] - 2024-10-22

### Added

- Dockerfile for containerised builds (#33811ec)
- Publish option chain results to Telegram channel (#32e635a)
- Upload reports to Dropbox (#0c845cf)
- Cloud Run job configuration (`job.yaml`) and `perform-all` mode (#2c65d09)

## [0.0.1] - 2024-10-13

### Added

- Initial implementation: pull stock quotes, calculate rate of return, compute ATR (#8249c7b, #414a7fc, #c3f311c)
- SQLite storage for candles and option strikes (#8afd875, #97b95fd)
- Symbol chunking for batched API requests (#515239c)
- Option chain retrieval based on ATR ranges (#28e804c)

[0.7.0]: https://github.com/compare/v0.6.2...v0.7.0
[Unreleased]: https://github.com/compare/v0.7.0...HEAD
[0.6.0]: https://github.com/compare/v0.5.6...v0.6.0
[0.5.6]: https://github.com/compare/v0.5.5...v0.5.6
[0.5.5]: https://github.com/compare/v0.5.4...v0.5.5
[0.5.4]: https://github.com/compare/v0.5.3...v0.5.4
[0.5.3]: https://github.com/compare/v0.5.2...v0.5.3
[0.5.2]: https://github.com/compare/v0.5.1...v0.5.2
[0.5.1]: https://github.com/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/compare/v0.0.3...v0.1.0
[0.0.3]: https://github.com/compare/v0.0.2...v0.0.3
[0.0.2]: https://github.com/compare/v0.0.1...v0.0.2
[0.0.1]: https://github.com/releases/tag/v0.0.1
