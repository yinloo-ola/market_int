...
- Culture: pure-function/hermetic tests; fixtures over mocks; the pipeline is
  ...
  happy-dom) asserting expansion/tooltips/states/rescore-interactions against
  the built bundle, plus `scripts/run-parity.mjs` pinning the JS scoring port
  (`src/lib/scoring.js`) to the Rust scorer (model.rs test vectors + the
  formula-true sample fixture regenerated by the Rust suite); no vitest/jest
  by decision.
...
- Tunables belong in `crates/core/src/constants.rs`
  (`MIN_RATE_OF_RETURN` 0.20, `API_BATCH_SIZE` 10, `PERFORM_ALL_SPY_TREND_RATIO`
...
- Scoring model: 20% Sharpe + 40% safety (max_drop band position, or
  ...
  ror < 0.20 and Sharpe ≤ 0. `score_components` on every result row comes
  from the same implementation as the total (property-tested).
- Client-side re-scoring (2026-09, `.scratch/rescore/`): the GUI re-weights
  persisted rows live in JS — `frontend/src/lib/scoring.js` is an exact port
  of the Rust scorer (parity-gated in the smoke lane); an App-level drawer
  (3 weight sliders + min-ror floor + reset) feeds both timeframes. At
  production defaults the view shows the frozen document scores verbatim;
  live scores apply only under adjusted weights, and a lowered floor
  re-admits persisted null-score rows. Rows dropped pipeline-side by the
  |delta| fence / earnings-upper-half rule are NOT in the document and can
  never re-surface client-side. The data view stays the table at every
  width (a mobile card view was prototyped, then reverted by user decision).
  The sample fixture is formula-true (its scores come from the real scorer
  via `regenerate_sample_last_run_fixture`).
